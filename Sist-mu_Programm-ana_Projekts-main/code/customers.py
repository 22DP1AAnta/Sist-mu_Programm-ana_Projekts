...
class Customers:
    ...
    def count_dicts_with_keyword(self, json_file, keyword):
            with open(json_file) as file:
                data = json.load(file)

            count = 0
            for item in data:
                if isinstance(item, dict) and item.get("name", "").lower() == keyword.lower():
                    count += 1

            print(f"Number of dictionaries containing the keyword '{keyword}': {count}")